...
from lba.util import file as fi
...
def main(datapath, out_path):
    ...
    dat = []
    with open(os.path.join(datapath, 'INDEX_general_NL.2020')) as f:
        for line in f:
            if line.startswith('#'):
                continue
            l = line.strip().split()
...
#     refined_set = refined_set[["-mer" not in l for l in refined_set.ligand]]
    ...
    refined_set.to_csv(os.path.join(out_path, 'pdbbind_NL_cleaned.csv'), index=False)

    labels = refined_set[['pdb', 'neglog_aff']].rename(columns={'neglog_aff': 'label'})

    labels.to_csv(os.path.join(out_path, 'pdbbind_NL_labels.csv'), index=False)
