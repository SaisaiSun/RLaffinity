import lba.util.voxelize as vox
...
def prot_graph_transform(item, atom_keys=['atoms'], label_key='scores'):
    """Transform for converting dataframes to Pytorch Geometric graphs, to be applied when defining a :mod:`Dataset <lba.datasets.datasets>`.
...
def voxel_transform(item, grid_config, rot_mat=None, center_fn=vox.get_center, random_seed=None, structure_keys=['atoms']):
    """Transform for converting dataframes to voxelized grids compatible with 3D CNN, to be applied when defining a :mod:`Dataset <lba.datasets.datasets>`.
    ...
    :type grid_config: :class:`dotdict <lba.util.voxelize.dotdict>`
